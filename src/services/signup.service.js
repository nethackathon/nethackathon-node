...
async function get(username) {
  const records = await db.query("SELECT pronouns, schedule, discord_username, notes from streamer where username = ?;",
    [username]);
  return {
...
async function updateText(username, notes, discordUsername, pronouns) {
  return await db.query("UPDATE streamer set pronouns = ?, notes = ?, discord_username = ? where username = ?;",
    [pronouns, notes, discordUsername, username]);
}
